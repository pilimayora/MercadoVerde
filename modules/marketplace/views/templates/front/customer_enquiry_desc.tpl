...
<span id="back"><a id="back_button" href="{$link->getModuleLink('marketplace', 'customer_queries')}">back</a><input type="hidden" value="{$base_dir}" id="base_url" /></span>
	<div class="view_enq">
	...
	<div id="send_mail">
		...
		<input type="hidden" name="id_cus" value="{$customer_id}" id="id_cus" />
	</div>	  
{assign var=j value=0}
	 ...
	 {else}
		 <div id="nego_data"><span>Customer:</span><p>{$query_records[$j]['description']}</p></div>
	 {/if}
...
var current_url = '{$customer_queries_link}';
$('#back_button').attr('href',current_url)
			...
			var my_id = {$my_id};
				...
				$.ajax({
								type: 'GET',
								url: '{$modules_dir}marketplace/mail_send_customer.php',
								async: true,
