			...
			<div class="seller_registration_form_content">
				<div class="inner_form_div">
						...
						<div id="outer_container">
							{if {$set}==1}
								<div class="row-info">	
									<div class="row-info-left">{l s='Choose Customer' mod='sellerpartner'}<sup>*</sup></div>	
							...
							{/if}
							<div class="row-info">	
								<div class="row-info-left">{l s='Shop Name' mod='sellerpartner'}</div>	
								<div class="row-info-right">
								<input class="reg_sel_input" type="text" id="shop_name1" name="shop_name" {if {$set}==0}value="{$shop_name}"{/if}/> <sup>*</sup>
								<label class="errors" id="shop_name_error">Required field.</label></div>	
							</div>	
							<div class="row-info">
								<div class="row-info-left">{l s='Shop Description' mod='sellerpartner'}</div>
								<div class="row-info-right1"><textarea name="about_business"  class="about_business" style="height: 100px;   padding: 2%;width: 550px;">{if {$set}==0}{$about_shop}{/if}</textarea></div>
							</div> <!--about_business--> 
							 
							
							 
							<div id="person_name" class="row-info" >
								<div class="row-info-left">{l s=' Seller Name' mod='sellerpartner'}</div>
								<div class="row-info-right">
								<input class="reg_sel_input"  type="text" name="person_name" id="person_name1" {if {$set}==0}value="{$seller_name}"{/if}/> <sup>*</sup><label class="errors" id="person_name_error">Required field.</label></div>
							</div> <!-- person_name--> 
							
							<div class="row-info">
								<div class="row-info-left">{l s='Phone' mod='sellerpartner'}</div>
								<div class="row-info-right">
								<input class="reg_sel_input" type="text" name="phone" id="phone1" maxlength="10" {if {$set}==0}value="{$phone}"{/if}/>
								<sup>*</sup><label class="errors" id="phone_error"></label></div>
							</div> <!-- phone-->
								 
									
						<div class="row-info">
							<div class="row-info-left">{l s='Fax' mod='sellerpartner'}</div>
							<div class="row-info-right"><input class="reg_sel_input" type="text" name="fax"  id="fax1" maxlength="10" {if {$set}==0}value="{$fax}"{/if}/><label class="errors" id="fax_error"></label></div>
						</div> <!-- fax--> 
						
						<div class="row-info">	
							<div class="row-info-left">{l s='Business Email' mod='sellerpartner'}</div>
							<div class="row-info-right"><input class="reg_sel_input" type="text" name="business_email_id" id="business_email_id1"  {if {$set}==0}value="{$business_email}"{/if} style="height:25px;"/><label class="errors" id="business_email_id_error" >Not a Valid Email Address</label></div>
						</div>  <!-- siteurl--> 
									
						<div class="row-info">	
							<div class="row-info-left">{l s='Address' mod='sellerpartner'}</div>
							<div class="row-info-right1"><textarea name="address" rows="6" cols="35" >{if {$set}==0}{$address}{/if}</textarea></div>
						</div>
						
						<div id="facebook" class="row-info" >
								<div class="row-info-left">{l s='Facebook Id' mod='sellerpartner'}</div>
								<div class="row-info-right"><input class="reg_sel_input"  type="text" name="fb_id" id="fb_id1" {if {$set}==0}value="{$facebook_id}"{/if}/></div>
							</div>
							
						<div id="twitter" class="row-info" >
								<div class="row-info-left">{l s='Twitter Id' mod='sellerpartner'}</div>
								<div class="row-info-right"><input class="reg_sel_input"  type="text" name="tw_id" id="tw_id1" {if {$set}==0}value="{$twitter_id}"{/if}/></div>
						</div>	
						{if {$add} == 0}
						{if {$set}==0}
								<div class="row-info">  
									<div class="row-info-left">{l s='Previous Shop Logo' mod='sellerpartner'}</div>
									<div class="prev_image" style="float:left;">
										<img src="{$shopimagepath}" width="100" height="100" />
						...
						<div class="row-info">  
							<div id="upload_logo" class="sell_row">
								{if {$set}==0}
									<div class="row-info-left">{l s='Change Shop Logo' mod='sellerpartner'}</div>
								{else}
									<div class="row-info-left">{l s='Upload Shop Logo' mod='sellerpartner'}</div>
								{/if}
						...
						{if {$add} == 0}
						<div class="row-info">  
									<div class="row-info-left">{l s='Previous Seller Logo' mod='sellerpartner'}</div>
									<div class="prev_image" style="float:left;">
										<img src="{$sellerimagepath}" width="100" height="100" />
									</div>
						</div>
						{/if}
						<div class="row-info">  
							<div id="upload_logo" class="sell_row">
								{if {$set}==0}
									<div class="row-info-left">{l s='Change Seller Logo' mod='sellerpartner'}</div>
								{else}
									<div class="row-info-left">{l s='Upload Seller Logo' mod='sellerpartner'}</div>
								{/if}
									<div class="row-info-right"><input class="reg_sel_input" type="file"  name="upload_seller_logo" /></div>
							</div> 
						</div>
						{if $set==1}
